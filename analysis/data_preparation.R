...
#get distribution of respondents' choice
table(data$v81)
...
dupl <- c()
for (i in data_inf$vid){
  k=sum(i == data_inf$vid)
  if(k==2){
    dupl <- c(dupl,i)
  }
}
dupl <- unique(dupl)
data[data$vid == 7712008,c("vid","v18.1","v18.2","v18.3","v18.4")]

library(ggplot2)
ggplot(data_inf, aes(variable))+geom_bar(color="gray")+
  xlab("Effective Number of Parties in Individual Voters' Effective Choice Sets")+theme_bw()

table(data_inf$variable)/1030
...
table(v81_dummy$X2,v81_dummy$value)
